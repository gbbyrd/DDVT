...
def unpatchify(x, channels=3):
    "This unpatchify function assumes the latent space dimensions are a square."
...
def unpatchify_rectangle(x, height, width, channels=3):
    "This unpatchify operation can be used with rectangle or square latent space."
    patch_size = int((x.shape[2] // channels) ** 0.5)
    w = width
    h = height
    assert h * w == x.shape[1] and patch_size ** 2 * channels == x.shape[2]
...
class UViT(nn.Module):
    def __init__(self, img_size=[64, 64], patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        # num_patches = (img_size // patch_size) ** 2 # original code
        num_patches = (img_size[0] // patch_size) * (img_size[1] // patch_size) # new code allows for rectangular images
    ...
    def forward(self, x, timesteps=None, context=None, translation_label=None, y=None, **kwargs):
        ...
        x = x[:, self.extras:, :]
        # the width and height for the unpatchify operation is the width and height of the
        # patched image.. so the width is the width of the latent space image divided by the patch size
        # and the height is the height of the latent space image divided by the patch size
        x = unpatchify_rectangle(x, self.img_size[0] // self.patch_size, 
                                 self.img_size[1] // self.patch_size, self.in_chans)
