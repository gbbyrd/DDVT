...
try:
    ...
    path = '/home/nianyli/Desktop/code/carla_versions/carla_0_9_14/PythonAPI/carla/dist/carla-0.9.14-py3.7-linux-x86_64.egg'
...
from carla_utils import generate_traffic, Weather

"""
This script is to collect data on Town01 with traffic. Proximity sensors will
be used to ensure the sensors are not spawned inside the vehicles or inside 
invalid locations.
"""
...
class CarlaSyncMode(object):
    ...
    def __init__(self, world, constant_ego_vehicle=False, **kwargs):
        self.world = world
        self.actors = []
        self.ego_vehicle = kwargs.get('ego_vehicle', None)
        self.vehicles = kwargs.get('vehicles', None)
        self.constant_ego_vehicle = constant_ego_vehicle
        self.frame = None
        ...
        if self.ego_vehicle is None and self.vehicles is None:
            raise Exception('Must specify ego vehicle or provide list of vehicles to CarlaSyncMode object')
        
        if self.ego_vehicle is None and self.constant_ego_vehicle:
            raise Exception('If constant_ego_vehicle is True, you must specify an ego vehicle' \
                'in the CarlaSyncMode object arguments')
    ...
    def __exit__(self, *args, **kwargs):
        ...
        print('cleaning up sync mode sensors')
        ...
        #     actor.destroy()
        print('finished cleaning up sync mode sensors')
    ...
    def randomize_auxiliary_sensors(self):
        ...
        # get a random vehicle if the ego vehicle is not kept constant
        if not self.constant_ego_vehicle:
            self.ego_vehicle = random.choice(self.vehicles) 
...
def main():
    ...
    # keep track of whether you have saved the labels.json file to avoid saving twice
    already_saved = False
    ...
    try:
        ...
        # define a range of traffic vehicles depending on the town
        traffic_quantity = {
            'Town01': [30, 60],
            'Town02': [30, 60],
            'Town03': [30, 60],
        ...
        if args.num_vehicles is not None or args.num_walkers is not None:
            vehicles_list, walkers_list, all_id, traffic_vehicle_actors = generate_traffic(client, world, number_of_vehicles=args.num_vehicles,
                            number_of_walkers=args.num_walkers)
        else:
            num_vehicles = random.randint(traffic_quantity[args.town][0], 
                                          traffic_quantity[args.town][1])
            num_walkers = random.randint(traffic_quantity[args.town][0]//3,
                                         traffic_quantity[args.town][1]//3)
            vehicles_list, walkers_list, all_id, traffic_vehicle_actors = generate_traffic(client, world, number_of_vehicles=num_vehicles,
                            number_of_walkers=num_walkers)
        
        ########################################################################
        # generate weather controls
        ########################################################################
        
        # create the weather class
        weather = Weather(world.get_weather())
        
        # set the weather changing variables
        speed_factor = args.weather_speed_factor
        ...
        # sleep for 10 seconds to give vehicles time to start moving
        time.sleep(10)
        
        with CarlaSyncMode(world, constant_ego_vehicle=False, 
                           fps=30, vehicles=traffic_vehicle_actors,
                           sensor_params=sensor_params) as sync_mode:
            ...
            aux_occluded_count = 0
            restart = False
            
            # run simulation and collect data
            while train_img_count < num_images:
                
                # if the auxiliary sensor has been occluded too many times in a row,
                # break out of the simulation
                if restart:
                    print('restarted because too many aux sensors were occluded')
                    break
                ...
                # the below if statement slows down the rate at which the weather
                # changes
                if frame_count % args.skip_frames * 1 == 0:
                    # update the weather  the below code is taked directly from the 
                    # PythonAPI/examples/dynamic_weather.py file
                ...
                vel = sqrt(sync_mode.ego_vehicle.get_velocity().x**2 + sync_mode.ego_vehicle.get_velocity().y**2)
                if vel < .001:
                    frame_count += 1
                    sync_mode.remove_sensors_from_queue()
                    sync_mode.randomize_auxiliary_sensors()
                ...
                for idx, data in enumerate(auxiliary_sensor_data):
                    ...
                    data_type = 'from'
                    train_img_info = get_img_info_dict(data_type, sensor_type, train_img_count, sync_mode.auxiliary_sensor_info[0])
                ...
                if not save_labels:
                    ...
                    aux_occluded_count += 1
                    # randomize sensor locations
                    sync_mode.remove_sensors_from_queue()
                    sync_mode.randomize_auxiliary_sensors()
                    sync_mode.randomize_target_sensors()
                    if aux_occluded_count == 10:
                        restart = True
                    continue
                ...
                for idx, data in enumerate(target_sensor_data):
                    ...
                    # add img info dict to sensor group list
                    data_type = 'to'
                    train_img_info = get_img_info_dict(data_type, sensor_type, 
                                                       train_img_count, sync_mode.target_sensor_info[idx])
                    sensor_group_data[f'{sensor_type}_img_{train_img_count}_info'] = train_img_info
                ...
                if save_labels:
                    ...
                    for idx, img_info in enumerate(imgs_to_save):
                        isSave = True
                        img_save_path = os.path.join(args.dataset_path,
                                                     img_info['img_name'])
                        for sensor_type in do_not_save:
                            if sensor_type in img_info['img_name']:
                                img_name = img_info['img_name'].split('.')[0]
                                img_type = img_name.split('_')[1]
                                img_num = int(img_name.split('_')[-1])
                                sensor_group_key = img_type + '_img_' + str(img_num) + '_info'
                                del sensor_group_data[sensor_group_key]
                                isSave = False
                                break
        ...
        # exact cleanup script from the original generate_traffic.py file
...
def verify_dataset():
    ...
    for saved_img_path in saved_img_paths:
        sensor_type = saved_img_path.split('_')[1:-1]
        if len(sensor_type) > 1:
            sensor_type = f'{sensor_type[0]}_{sensor_type[1]}'
        else:
...
def clean_dataset():
    ...
    print(f'Deleted {len(deleted_images)} images not found in json file.')
...
def visualize_occlusions(args):
    ...
    img_paths.sort()
    results = []
    for depth_img_path in tqdm(img_paths):
...
if __name__=='__main__':
    ...
    parser.add_argument(
        ...
        default='Town01',
        type=str,
        help='Specify the town to collect data on')
    ...
    parser.add_argument(
        ...
        action='store',
        type=int,
        help='Specify the number of traffic vehicles to spawn.')
    
    parser.add_argument(
        '--num_walkers',
        action='store',
        type=int,
        help='Specify the number of pedestrian walkers to spawn.')
